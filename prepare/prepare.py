...
from json2label import json2labelImg
...
def gen_labelColors(json_file):
    # label_file = json_file.replace("annotations","labels").replace(".json", "_labelTrainIds.png")
    label_file = json_file.replace("annotations","labels_color").replace(".json", ".png")
    label_dir = os.path.dirname(label_file)
    if not os.path.exists(label_dir):
        os.mkdir(label_dir)
...
def main():
    ...
    color_map_path = os.path.join(fine_path, 'labels_color')
    ...
    try:
        ...
        print(f'labels have been saved in `{label_map_path}`')
        print(f'labels with color have been saved in `{color_map_path}`.')
